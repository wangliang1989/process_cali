...
# 解压mseed文件
foreach my $mseed (glob "*.mseed") {
    #2018-08-15.CI.MUR.mseed     CI_CSH.dataless
    my ($time, $net, $sta) = split m/\./, $mseed;
    my $dataless = "../dataless/${net}_${sta}.dataless";
    die "can not find $dataless" unless (-e $dataless);
    system "rdseed -Rdf $mseed -g $dataless";
}
